        ...
        <div class="table-responsive">
          <table id="table_data" class="table table-bordered table-admin">
            <tr align="center">
              ...
              <td rowspan="2">Jumlah penumpang Total</td>
              <td rowspan="2">Ketersedian bus</td>
            ...
            foreach ($data_loket->result_array() as $s) {
              $s['jumlah_penumpang_total'] = $this->db->query('SELECT COUNT(no_jumlah_tujuan) as jumlah_penumpang FROM jumlah_tujuan_loket WHERE no_loket = ' . $s['no_loket'])->row()->jumlah_penumpang;
              $s['ketersediaan_bus_total'] = $this->db->query('SELECT SUM(jumlah_bus) as jumlah_bus FROM jumlah_bus_loket WHERE no_loket = ' . $s['no_loket'])->row()->jumlah_bus;
              $s['jumlah_fasilitas_total'] = $this->db->query('SELECT COUNT(no_jumlah_fasilitas) as jumlah_fasilitas FROM jumlah_fasilitas_loket WHERE no_loket=' .  $s['no_loket'])->row()->jumlah_fasilitas;
            ?>
              <tr>
                <td><?php echo $s['no_loket']; ?></td>
                <td><?php echo $s['nama_loket']; ?></td>

                <td><?php echo $s['jumlah_penumpang_total'];
                    ?></td>
                <td><?php echo $s['ketersediaan_bus_total'];
                    ?></td>
                <td><?php echo $s['jumlah_fasilitas_total'];
                    ?></td>

                <td colspan="3"><?php
                                $hc1 = sqrt(pow(($s['jumlah_penumpang_total'] - $c1a), 2) + pow(($s['ketersediaan_bus_total'] - $c1b), 2) + pow(($s['jumlah_fasilitas_total'] - $c1c), 2));
                                echo $hc1;
                                ?></td>
                <td colspan="3"><?php
                                $hc2 = sqrt(pow(($s['jumlah_penumpang_total'] - $c2a), 2) + pow(($s['ketersediaan_bus_total'] - $c2b), 2) + pow(($s['jumlah_fasilitas_total'] - $c2c), 2));
                                echo $hc2;
                                ?></td>
                <td colspan="3"><?php
                                $hc3 = sqrt(pow(($s['jumlah_penumpang_total'] - $c3a), 2) + pow(($s['ketersediaan_bus_total'] - $c3b), 2) + pow(($s['jumlah_fasilitas_total'] - $c3c), 2));
                                echo $hc3;
                                ?></td>
                <?php
                ...
                $arr_c1_temp[$no] = $s['jumlah_penumpang_total'];
                $arr_c2_temp[$no] = $s['ketersediaan_bus_total'];
