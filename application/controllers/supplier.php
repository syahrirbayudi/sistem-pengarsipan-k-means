...
class Supplier extends CI_Controller
{
	...
	function generate_awal()
	{
		...
		if ($sesinya['level'] != '3') {
			...
			$data_loket = $this->suppliermodel->selectdata('data_loket');
	...
	function generate_rata()
	{
		...
		if ($sesinya['level'] != '3') {
			...
			$data_loket = $this->db->get('data_loket');
			$v = "";
			if (count($data_loket->result()) < 0) {
				$nilai = floor(($s->jumlah_penumpang_total + $s->ketersediaan_bus_total + $s->jumlah_fasilitas_total) / 3);
				$v = "insert into rata_rata (no_loket,rata_rata) values ('" . $s->no_loket . "','" . $nilai . "')";
				$this->db->query($v);
			} else {
				$this->db->query('truncate table rata_rata');
				foreach ($data_loket->result() as $s) {
					...
					$nilai = floor(($jumlah_penumpang_total->row()->jumlah_penumpang + $ketersediaan_bus_total->row()->jumlah_bus + $jumlah_fasilitas_total->row()->jumlah_fasilitas) / 3);
					$v = "insert into rata_rata (no_loket,rata_rata) values ('" . $s->no_loket . "','" . $nilai . "')";
					$this->db->query($v);
				}
			}
			...
			$data['data_loket'] = $this->db->query('select * from data_loket left join rata_rata on data_loket.no_loket=rata_rata.no_loket');
	...
	function generate_centroid()
	{
		...
		if ($sesinya['level'] != '3') {
			...
			$data['c4'] = rand(40, 59);
			$data['c5'] = rand(0, 39);
			$data_loket = $this->db->query('select * from data_loket left join rata_rata on data_loket.no_loket=rata_rata.no_loket');
			$st = "";

			$this->db->query('truncate table hasil');
			foreach ($data_loket->result() as $s) {

				$jumlah_penumpang_total = $this->db->query('SELECT COUNT(no_jumlah_tujuan) as jumlah_penumpang FROM jumlah_tujuan_loket WHERE no_loket="' . $s->no_loket . '"')->row()->jumlah_penumpang;
				$ketersediaan_bus_total = $this->db->query('SELECT SUM(jumlah_bus) as jumlah_bus FROM jumlah_bus_loket WHERE no_loket = ' . $s->no_loket)->row()->jumlah_bus;
				$jumlah_fasilitas_total = $this->db->query('SELECT SUM(jumlah_fasilitas) as jumlah_fasilitas FROM jumlah_fasilitas_loket WHERE no_loket = ' . $s->no_loket)->row()->jumlah_fasilitas;
				...
				$this->db->query("insert into hasil (no_loket,predikat,d1,d2,d3,d4,d5) values('" . $s->no_loket . "','" . $st . "','" . $d1 . "','" . $d2 . "','" . $d3 . "','" . $d4 . "','" . $d5 . "')");
			}



			$data['data_loket'] = $this->db->query("select * from data_loket left join (rata_rata,hasil) on data_loket.no_loket=rata_rata.no_loket and data_loket.no_loket=hasil.no_loket");

			$data['jumlah_penumpang_total'] = $jumlah_penumpang_total;
			$data['ketersediaan_bus_total'] = $ketersediaan_bus_total;
			$data['jumlah_fasilitas_total'] = $jumlah_fasilitas_total;
			$this->load->view('supplier/header', $data);
	...
	function iterasi_kmeans_lanjut()
	{
		...
		if ($sesinya['level'] != '3') {
			...
			$data['data_loket'] = $this->db->get('data_loket');
			$id = "";
...
<?php
	...
	function iterasi_kmeans_hasil()
	{
		...
		if ($sesinya['level'] != '3') {
			...
			$data_hasil = $this->suppliermodel->selectdata('hasil INNER JOIN data_loket on hasil.no_loket = data_loket.no_loket order by d5 DESC');
